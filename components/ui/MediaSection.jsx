...
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/landingcarousel';
  ...
  return (
    <div className="px-3 md:px-10  lg:mt-16 relative">
      <div>
        ...
        >
          Connect With Us <br></br> on  <span className={greatVibes.className}>Social</span>  Media

        </div>
        <p className=' max-w-2xl mt-2 mx-auto text-center '>
      ...
      >
        <CarouselContent >
          {clientInfo?.mediaItems.map((item, index) => {
            ...
            const className = isReel
              ? "w-full -mr-1 scale-150"
              : "w-full -mt-12";

            return (
              <CarouselItem key={index} className="basis-full sm:basis-1/2 md:basis-2/3 lg:basis-[25%] overflow-hidden">
                <div
                  className={`bg-white relative shadow-lg border rounded-xl ${isReel ? "p-2.5" : "pt-1 px-2.5"
                    } overflow-hidden`}
                ...
                >
                  ...
                  <div
                    className={`relative rounded-xl transition-all flex w-full justify-center ${isReel ? "bg-black" : "bg-white"
