        ...
        <div className=" mt-16 space-y-16 md:mt-24 md:space-y-24">
          {entries.map((entry, index) => {
            ...
            return (
              <div
                key={index}
                className="relative flex flex-col gap-4 md:flex-row md:justify-between w-full  md:gap-16"
                ref={(el) => setItemRef(el, index)}
                aria-current={isActive ? "true" : "false"}
              >
                {/* Sticky meta column */}
                <div className="top-8 flex h-min  w-1/2 shrink-0 items-center gap-4 md:sticky">
