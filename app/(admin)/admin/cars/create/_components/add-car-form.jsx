...
export const AddCarForm = () => {
  ...
  const onMultiImagesDrop = useCallback((acceptedFiles) => {
    const validFiles = acceptedFiles.filter((file) => {
      if (file.size > 50 * 1024 * 1024) {
        toast.error(`${file.name} exceeds 50MB limit and will be skipped`);
        return false;
      }
      return true;
    });
    ...
    <div>
      ...
      >
        <TabsList className="grid w-full grid-cols-2 h-14">
          <TabsTrigger value="manual" className=" h-full  ">Manual Entry</TabsTrigger>
          <TabsTrigger value="ai" className=" h-full ">AI Upload</TabsTrigger>
        </TabsList>
